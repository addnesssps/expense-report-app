...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        output = _generate_excel(expenses, applicant, company, period_start, period_end)
...
def _generate_excel(expenses, applicant, company, period_start, period_end):
    wb = Workbook()
    ws = wb.active
    ws.title = "経費精算書"
    ...
    for i, exp in enumerate(expenses, 1):
        ...
        total_amount += amount
        values = [i, exp.get("date", ""), exp.get("vendor", ""),
                  exp.get("category", ""), amount, exp.get("description", "")]
    ...
    row += 1
    for col_idx, h in enumerate(["科目", "件数", "金額"], 1):
        cell = ws.cell(row=row, column=col_idx, value=h)
        cell.font = header_font
        cell.fill = header_fill
        cell.alignment = center
